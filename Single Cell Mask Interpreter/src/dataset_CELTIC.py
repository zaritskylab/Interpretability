...
class CELTICDataGen(Dataset):
    def __init__(self, csv_path, image_dir, context_csv_path=None, transforms=None, signals_are_masked=False):
        self.df = pd.read_csv(csv_path)
        self.image_dir = image_dir
        self.context_df = pd.read_csv(context_csv_path) if context_csv_path else None
    ...
    def __getitem__(self, idx):
        ...
        if self.context_df is not None:
            context_vector = torch.tensor(self.context_df.iloc[idx].values.astype(np.float32))
        else:
            context_vector = torch.zeros((0,), dtype=torch.float32)

        return (signal, context_vector), target, mask