...
class MaskGenerator(nn.Module):
    ...
    def forward(self, signal, target, m):
        # --- CELTIC prediction on-the-fly ---
        prediction = self.celtic_wrapper(signal, m)
        ...
        unet_prediction = self.celtic_wrapper(adapted_image, m)
    ...
    def train_step(self, inp, target, m, optimizer):
        self.train()
        optimizer.zero_grad()
    
        with autocast():
            loss, mask, pcc = self.forward(inp, target, m)
    ...
    @torch.no_grad()
    def test_step(self, inp, target, m):
        self.eval()
        loss, mask, pcc = self(inp, target, m)
        return {
            "val_loss": loss.item(),
            "val_pcc": pcc.item(),
        }
