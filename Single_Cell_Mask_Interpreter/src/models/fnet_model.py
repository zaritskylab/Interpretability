...
class CELTICModel(object):
    ...
    def do_train_iter(self, signal, target, mask=None, tabular_signal=None):
        ...
        if self.signals_are_masked:
            foreground = torch.where(mask.flatten() != 0)
            loss = self.criterion(output.flatten()[foreground], target.flatten()[foreground])
        else:
    ...
    def predict(self, signal, tabular_signal=None):
        ...
        module.eval()
        with torch.no_grad():
            prediction = module(signal, tabular_signal).cpu()
        return prediction
