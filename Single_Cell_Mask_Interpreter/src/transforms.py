...
def normalize_with_mask(img, mask):
    """Subtract mean, set STD to 1.0, on mask only"""
    mask_indices = np.where(mask!=0)
    mask_items = img[mask_indices].astype(np.float64)
    mask_mean = np.mean(mask_items)
    mask_std = np.std(mask_items)
    result = np.where(mask!=0, ((img - mask_mean) / mask_std), 0)
    return result
...
class Propper(object):
    ...
    def __repr__(self):
        return 'Propper({})'.format((self.action, self.transformer))
...
class Padder(object):
    def __init__(self, padding='+', by=16, mode='constant', constant_values=0, min_final_shape=None):
        """
        ...
        self.min_final_shape = min_final_shape
    ...
    def _calc_pad_width(self, shape_in):
        ...
        for i in range(len(shape_in)):
            if isinstance(paddings[i], int):
                ...
                if not self.min_final_shape:
                    shape = shape_in[i]
                else:
                    # if one of the shapes is smaller than self.by, and the self.by is smaller than the patch size, the result will be smaller than a patch.
                    # by providing the patch size as min_final_shape, the minimal patch shape is guaranteed.
                    shape = np.max([shape_in[i], (self.min_final_shape[i] - self.by) + 1   ])
                padding_total = int(np.ceil(1.*shape/self.by)*self.by) - shape_in[i]
    ...
    def __call__(self, x_in):
        ...
        x_out = np.pad(x_in, pad_width, mode=self.mode, constant_values=self.constant_values)
    ...
    def __repr__(self):
        return 'Padder{}'.format((self.padding, self.by, self.mode, self.constant_values, self.min_final_shape))
